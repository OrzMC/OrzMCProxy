...
## 5b. 带宽瓶颈与规格选型（2026-08-14 沉淀）

**链路瓶颈 = 家宽上行**（MC 流量以下行为主，恰好打在家宽上行这一环）：

```
玩家 ←→ 中转机(公网带宽) ←→ 家宽上行 50M ←→ 家里服务器
```

中转机带宽超过家宽上行的部分在出网方向**用不到**——所以选型核心是**计费方式**而非带宽数字：

| 计费方式 | 100M 是否浪费 | 说明 |
|:--|:--|:--|
| **按量 CVM 100M 峰值**（流量计费） | ✅ 不浪费 | 峰值只是能力上限，**按实际流量付费**（~0.8 元/GB）；实际只跑 ~50M 就只付 50M 的钱；还能扛玩家→服务器的突发入站 |
| **轻量固定 100M 带宽**（月付） | ❌ 浪费 | 每月固定付 100M 的钱，实际只有 50M 能跑出去 |

**家宽 50M 上行容量**（带宽不是 50 人活动的瓶颈，瓶颈在 MCSM 单服并发/插件/内存）：

| 在线人数 | 需中转机出网带宽 | 50M 上行占用 |
|:--|:--|:--|
| 15 人 | ~3M | 6% |
| 30 人 | ~4.5M | 9% |
| 50 人 | ~10M（含峰值余量） | 20% ✅ 舒适 |
| 100 人 | ~20M | 40% 仍可行 |

**规格推荐**：日常 ≤30 人 → 轻量 2C2G 6M（长期）；50 人活动正日 → **按量 CVM 100M 峰值**（~18 元/天 + 实际流量费，≈32 元/天，关机不收费）；100 人双服 → 必须按量 CVM 100M。2C2G 所有场景都够（frps 纯转发，CPU/内存非瓶颈）。切换 = frpc 只改 serverAddr 一行（5 分钟），活动机可提前预置多实例。

## 6. 已知边界

- **家宽上行 50M 是链路硬上限**：均值占用 50 人≈20%、100 人≈40%；但**区块加载/传送等瞬间峰值可达 2–3 倍均值**（100 人峰值 20–30M，贴近 50M）——活动前必须压测验证（stress-stay.js 100 bot + 观察上行利用率）
